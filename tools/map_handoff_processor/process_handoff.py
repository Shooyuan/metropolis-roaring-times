...
def process(source_zip: Path, output_dir: Path, price_seed: str) -> dict:
    ...
    report = {
        ...
        "knownFollowUp": [
            "地标横幅 SVG 仍声明 Jacquard 24；接入网页前须随项目提供该字体或由项目负责人批准替代字体。",
            "仓库中的现有 .fig 本地副本早于本次 ZIP；项目负责人仍须保存与 v004 内容同步的新版本化 .fig 快照。",
...
Figma 的 `cheap`、`medium`、`expensive` 父组是项目负责人确定的价格层级，不是视觉分组。精确基础地价由处理器在内容构建时生成：便宜 `$6,000–$10,000`、中等 `$11,000–$18,000`、昂贵 `$19,000–$26,000`，步长 `$500`。种子与几何路径共同决定结果，因此同一版本重复处理得到完全相同的价格；进入游戏后不会每局改变。
...
- 横幅仍使用 Figma 导出的 `Jacquard 24` 文字声明；正式接入网页前必须把该字体纳入项目或由项目负责人批准替代。
- 仓库内现有 `.fig` 本地副本早于本次 ZIP；项目负责人仍须从当前 Figma 文件保存一份与 `v004` 同步的版本化 `.fig` 快照。
- 本阶段只生成运行素材和审查页，不修改当前网页，也没有把地块交互接进游戏。
- 历史地标的年代适配与百科内容属于后续内容阶段，本次仅保持项目负责人提交的位置和图像。
"""
