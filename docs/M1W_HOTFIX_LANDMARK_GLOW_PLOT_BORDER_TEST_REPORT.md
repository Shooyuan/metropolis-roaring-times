...
5. 2026-09-12 追加修正：桥梁等复杂线稿素材不得使用最外层 14px 大半径 `drop-shadow`，否则会把图片透明通道外接范围扩散成红雾块；地标反馈保留红色辉光，但不得再把建筑本体整体提亮或染色。
...
- `m0_web/styles.css`
  ...
  - 把地标辉光从建筑本体提亮改回纯红色外辉光，并移除最外层大半径扩散阴影。
...
- 桥梁类长条线稿不出现过度外扩的整块矩形红雾；如果需要更强反馈，优先清理透明素材或另做专用遮罩，而不是恢复最外层大半径阴影。
- 可购买地块边框比上一版明显更细，不遮挡道路和地图细节。

## 风险

辉光效果依赖透明插画素材。即使素材边角透明，桥梁等复杂线稿在过大半径 `drop-shadow` 下也会因为线条密集而形成矩形红雾；因此网页原型保留红色外辉光，但禁止最外层过度扩散和建筑本体整体提亮。若后续某个历史地标图片带有不透明底色或需要更强辉光，应回到素材源文件重导透明图或制作专用遮罩，而不是用程序猜建筑轮廓。
