...
5. 2026-09-12 追加修正：地标不再对原始建筑图使用 CSS `drop-shadow`；程序为每座地标生成独立透明红光遮罩图，悬停和选中时只显示红光层，避免浅色矩形底、边框或低透明底纹一起发光。
...
红光遮罩是运行时派生素材，必须随历史地标插画一起重新生成。如果老板后续重导 Figma 地标包或替换任一地标插画，需要重新运行 `tools/map_handoff_processor/build_landmark_glow_assets.py`，否则红光遮罩会和新插画不匹配。
