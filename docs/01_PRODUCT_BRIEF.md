...
- 最高缩放提高到 `1500%`；`500%` 是精细交互起点，`1250%` 是历史地标横幅显示起点，详细层级和交互阈值以 `04_MAP_AND_ART_DIRECTION.md` 为准。
