...
## M1W-UI-ADVICE-02 — 适宽地图与投资建议联动（2026-09-17）

- 按项目负责人本轮授权直接优化现有网页。地图初始、最低和复位比例按实际视窗适宽计算；保留最高 1500% 与 500% 细节交互阈值。
- 投资建议保留 150px / 矮窗口 138px 折叠高度；无消息显示指定开局简报，后续只保留最新一条，正文按框内空间调整字号。420ms 上推动画只更新新闻条，不重画地图。
- 投资建议仅在左栏向上展开，操作台同步缩至 87px 的标题与四个页签；点击操作台页签恢复。展开后可滚动查看历史消息。
- 左栏标题使用 `#1c2b1f` 白字，移除操作台收缩键；以方角、深色硬阴影、实色色块强化现有新粗野主义方向。
- 自动验证通过：JavaScript 语法、`ui_behavior_test.js`、`m1w_static_test.js`、`m1w_i18n_static_test.js`、`runtime_map_integration_test.js`、`git diff --check`。
- 浏览器实测：1600×1000 和 1280×720。适宽地图与视窗宽差小于 1px；加减与键盘复位遵守下限；展开保持左栏等宽、底边一致，操作台终态 87px。英文与中文开局简报可完整显示。
- 新闻实测推进至第 4 回合：过渡中旧条 `translateY(-11.94px)`、新条 `translateY(82.06px)`，两条位置差等于 94px 新闻视窗高度；过渡后仅一个新闻节点，正文无溢出，历史列表可见，浏览器无警告或错误。
- 此次不包含未提交的图片素材；Godot 和玩法范围不变。
...
- 新任务接手后重新读取项目规则、总纲、开发日志、项目负责人审查计划、Git 状态和最近提交；
- 确认当前主线事实仍是 M1W 制作人网页原型，Godot 正式玩法和最终 UI 尚未启动；
- 记录工作区未跟踪素材分组和项目负责人决策：旧 `m0_web/assets/cutouts/panel_center_large.png` 删除可保留，`references/` 未跟踪素材按长期参考处理，未归属 UI 素材暂不提交；
- 新增 M1W 网页视觉验收清单，下一步优先让项目负责人审查当前网页地图、地块、地标、缩放、互斥选择和双语文案。
- 2026-09-13 项目负责人确认：历史地标贴图已经接入，且 52 个历史地标摆在地图上的位置正确；后续不再把“历史地标位置”列为未确认项。

## M1W-HOTFIX-14 — 隐藏历史地标地图横幅

- 项目负责人决定不再尝试修历史地标地图横幅可读性，要求直接去掉地图上的历史地标标签名；
- M1W 运行时不再创建 `landmark-label` 图片层，也不再保留 `1250%` 标签显示门禁；历史地标插画、红光 hover/选中状态、点击命中和右侧 Historical File 名称正文保持可用；
- 横幅 SVG 仍作为 v004 运行素材和来源审计材料留在素材包中，但不参与网页地图渲染；
- 页面提示、双语文案和自动测试同步改为“地块与地标从 `500%` 起可选，历史地标地图标签隐藏”。

## M1W-HOTFIX-15 — 启动页与对手弹窗面板贴图替换

- 项目负责人指出启动页 Config / 对手弹窗面板贴图再次缺失；
- 启动页子面板、对手选择弹窗和三个对手选项统一改用 `m0_web/assets/new ui/01_plain_panel_2x.png`；
...
- 项目负责人确认地块可读性和通用建筑贴图落点正确，但指出历史地标横幅太小、地标 hover 出现边框和白色底图、地图拖动及点击地标/地块明显卡顿；
- 诊断确认白底和边框来自全局 `button:hover:not(:disabled)` 样式泄漏到地标按钮；
- 地标按钮增加高 specificity 覆盖，保持透明背景、无 outline、无阴影、无按压位移；
- 历史地标横幅放大到 `scale(2.2, 1.9)`，并增加浅色阴影提高可读性；
- 地图拖动改为 `requestAnimationFrame` 节流，拖动开始时缓存 pan 边界，拖动期间暂停地标红光、街区/地块描边和相关 transition，降低重绘压力；
- 自动测试和浏览器复查通过；其中横幅放大方案已在 `M1W-HOTFIX-14` 中被项目负责人新决定取代，当前运行时不再显示历史地标地图横幅。

### 风险与后续门槛

- 未跟踪 UI 素材中可能包含 U1 或后续正式 UI 资产，但项目负责人尚未决定哪些进入项目；
- 参考素材体积较大，若未来提交，需要单独做来源和仓库体积审查；
- M1W 视觉验收只判断当前网页原型可读性，不授权 Godot M1A、最终 UI 或 20 回合正式平衡。

## M1W-COPY-02 — 玩家文案正式化与 52 个历史地标正文接入

- 按用户审稿结果，把 52 个历史地标短介绍统一为“建筑名，单句描述”，并将已认可的中文长介绍接入网页运行时；
- 新增 `m0_web/landmark-content.js`，为 52 个运行 ID 提供 EN/ZH 名称、短介绍、长介绍、English Wikipedia 页面标题与逐项 URL；
- 新增 `scripts/sync_landmark_copy_from_draft.js`，以 `docs/LANDMARK_COPY_DRAFT.md` 为中文正文来源，防止文档与游戏内容分叉；
- 将 `docs/COPY_DECK.md` 中用户修改的中文同步至英文、语言包与 HTML，并把 12 个街区说明改为玩家视角；
- 清除玩家可见的“项目负责人绘制”“等待批准”“制作人演示”“正式制作输入”等内部工程身份文案；内部项目管理和审批记录继续留在相应文档中；
- 自动门禁新增 52 条双语内容完整性、短介绍格式、逐项英文 Wikipedia 链接和玩家文案禁词检查。

### 风险与后续门槛

- 15 个地标仍保留“待核对”的内部来源风险；这不会显示给玩家，但在正式发布前仍需逐项确认建筑身份、年代与地图位置；
- 当前来源链接在浏览器新标签页打开，而项目长期规格曾提出游戏内来源弹窗；此差异不属于本次文案阶段，后续 UI 阶段需统一。
...
- 第一次改为本体提亮后不符合项目负责人审美，已改回纯红色外辉光；
- 网页原型保留旧版红色辉光的方向，但移除最外层过度扩散阴影，并禁止建筑本体整体提亮或染色；
- 静态测试和运行地图集成测试新增防回归检查，禁止恢复最外层大半径地标阴影和本体提亮方案。

## M1W-HOTFIX-12 — 独立历史地标红光遮罩

- 项目负责人确认仍能看到浅色矩形底和边框感，要求变通处理红光外的矩形底；
...
- 当前排版表基于已导入的 v004 街区边界做近似适配；如果项目负责人后续重画街区几何，需要重新审查并微调每个标签位置；
- 极窄或不规则街区无法只靠字体自动排版保证完美居中，必要时应在 Figma 中增加人工标注锚点或给程序提供项目负责人批准的标签锚点表。
...
- 项目总纲、产品简报、垂直切片范围、地图美术规范、技术架构、验收标准、地图交付管线和项目负责人审查计划均已同步。
...
- 先整理当前 M1W 的历史地标简介和全量文案定位，方便项目负责人后续修改、增加和删除；
...
- 这些简介尚未写入游戏运行时，需项目负责人审稿后再进入单独实现阶段；
...
- `localization.css` 切换到项目负责人指定的两份 OTF 文件；
...
- 两份 OTF 合计约 23MB，会增加网页包体；当前先按项目负责人指定执行，公开部署前可再评估是否需要子集化；
...
- 对手选择弹窗必须恢复项目负责人提供的纸张面板贴图，不允许出现破图或透明底；
...
- 旧路径文件的删除属于项目负责人此前已有的工作区改动，本次不擅自把该删除并入提交；新路径素材作为本修复必要输入单独纳入版本控制。

## M1W-MAP-04 — V004 地块与历史地标网页接入

### 项目负责人决定

- 项目负责人批准采用已验收的 v004 轻量运行包，继续升级现有 M1W 制作人网页；
- 本阶段只验证正式几何、LOD、互斥选择和既有地产操作，不进入最终 U3/M2 界面，也不启动 Godot；
- 历史正文必须以后从英文 Wikipedia 单一来源整理并单独审批，本阶段不得编造。

### 本阶段实现

- 网页底图切换到 v004 运行包，加载 12 个街区、30 个项目负责人绘制的可购买地块和 52 个项目负责人放置的历史地标；
...
- 项目负责人必须在浏览器中审查实际地块、地标、横幅和缩放可读性，批准后才能申请下一阶段。

## MAP-RUNTIME-01 — V004 地图交付轻量化与地价固化

### 项目负责人决定

- 项目负责人授权 Codex 解决 1.2GB 导出包、运行 ID 异常与地价生成问题；
- Figma 的 `cheap | medium | expensive` 不是错误命名，而是项目负责人指定的地块价格层级；
...
- 生成接触表、离线对齐页、处理报告、中文项目负责人验收单和 V004 接收报告；
- 运行包约 17MB，相对 1.2GB 源 ZIP 缩减约 98.63%，现有网页未修改。

### 风险与门槛

- 自动测试通过不等于视觉批准；项目负责人必须检查接触表与对齐页后才能接入网页；
- 横幅依赖 `Jacquard 24`，接入前必须解决字体随包交付或替代审批；
- 仓库内现有 `.fig` 快照早于 V004 ZIP，项目负责人仍须保存同步版本的本地副本；
...
- 项目负责人在真实八层 Figma 地图中确认：默认快速模式每次进入 `07_HISTORICAL_LANDMARKS` 都会卡住；
- 图层审查确认每座地标已经使用独立父组，内部标题牌、文字与插画层级合理，卡顿不是由 `title` 或 `Rectangle 6` 等子层名称造成；
- 项目负责人批准优先修复风险，不修改 Figma 原稿和现有网页。
...
- 自动测试不能模拟 Figma 对真实图片执行 `exportAsync` 的内存行为。提交后仍须由项目负责人关闭并重新打开开发插件，再以默认快速模式完成一次真实 ZIP 下载；未通过现场复测前，本修复只算代码完成，不算地图素材验收完成。
...
- 项目负责人不接受继续等待，批准升级插件并提供不依赖整图合成的替代交付路径。
...
- 本地测试只能验证导出计划、八层识别、ZIP 生成和静态合同，无法替代项目负责人实际 Figma 文件中的导出性能；
- 项目负责人必须重新打开开发插件，以加载修改后的本地代码，并用默认快速模式完成一次 ZIP 下载；
...
- 只更新总纲、规则、范围、地图/美术、架构、验收、计划、地图交付和项目负责人审查文档；
- 新增 `IN_GAME_UI_LAYOUT_SPEC.md` 和项目负责人布局的小尺寸审阅图；
- 不修改当前 `m0_web` 页面，不生成新 UI 图，不开始 Godot 或 M2 实现；
- 当前 M1W 五页签界面只作为历史原型保留，不能再作为最终游戏内 UI 权威。

## DOC-MAP-CONTENT-01 — 项目负责人绘制地块与历史地标规则贯通

### 项目负责人决定

- 12 个 Financial District、SoHo、East Village 等区域统一称“街区”；青绿色闭合形状称“地块”；历史建筑与公共设施称“历史地标”；
- 正式地块只来自项目负责人的 `08_PURCHASABLE_BLOCK_GEOMETRY`，数量不预设，Codex 不按道路自动生成地块；
- `07_HISTORICAL_LANDMARKS` 保存项目负责人绘制的历史地标插画与英文横幅；地标长期显示，交互按缩放 LOD 开放；
...
- 历史地标玩法影响延期，进入相关阶段前必须再次提醒项目负责人审批。

### 本阶段边界

- 只更新权威文档、审批模板、中文名和项目负责人提交的 `assets/wikipedia-w.svg`；
- 不修改项目负责人 Figma，不接入尚未完成的 07/08 图层，不制作五套开发建筑插画，不改变当前 M1W 地图交互；
...
- 项目负责人批准把已认可的英文版和简体中文版立即合并，避免后续 UI 修改需要维护两套结构；
...
- 浏览器控制台无警告或错误；未修改项目负责人地图、Brand 或分区几何。

### 阶段决定

- 合版实现和内部验收完成；本阶段创建独立 Git 提交后停止；
- 等待项目负责人视觉审查和是否进入下一阶段的明确批准；
...
- 用项目负责人正式地图替换临时曼哈顿轮廓，没有在正式地图上显示或猜测旧18地块；
...
- Chelsea 使用项目负责人批准的原始 Figma 矢量网络，未重画边界；
...
- 内部浏览器305%实际渲染尺寸验证通过，底图与分区层的 `x/y/width/height` 差值全部为0，Chelsea 选择保持通过；Safari 视觉效果仍由项目负责人刷新后复核；
...
- M1W 实现与内部测试完成；
- 完成 Git 提交后停止，等待项目负责人视觉验收；
- 未获项目负责人明确批准前不得进入 Godot M1A，也不得创建64个正式地块。

## M1W-INTAKE-03 — Figma 交付包 v003 与正式 Brand 接收

### 项目负责人决定与本阶段动作

- 项目负责人提交 `metropolis_handoff_1786627051152.zip` 与同步更新的 `.fig` 本地副本；
- 项目负责人确认 `district_chelsea` 在 Figma 中检查无视觉问题，批准保留原始矢量网络；M1W 必须专项验证其悬停、点击和选中保持命中范围；
- 项目负责人确认 `assets/00_BRAND.svg` 是独立最终 Brand，并授权 Codex 直接补入标题冒号，不需要把 `00_BRAND` 放回地图 Figma；
...
- 完成本阶段测试与 Git 提交后停止，等待项目负责人明确批准开始 M1W 网页开发。
...
- 若以后重新引入桥梁溢价，必须由项目负责人批准明确地块清单，不得按旧分区名暗中恢复。

### Chelsea 修正说明

- 机器检查发现 `district_chelsea` 不是单一以 `Z` 结尾的闭合轮廓，而是 Figma 矢量网络；
- 项目负责人已在 Figma 中人工检查并确认视觉无误，项目不再要求重画或连接该边界；
- Codex 不修改权威几何，改在 M1W 中验证实际悬停、点击和选中保持范围；若失败则报告项目负责人重新裁决。
...
- Git 空白检查通过，项目负责人新 ZIP、`.fig` 和美术素材仍保持未提交。
...
- 项目负责人随后已通过 `DOC-MAP-12` 确认 12 区正式替代旧六区；本条保留为 v002 复验当时的历史状态。

### 阶段决定

- v002 解决一项、保留三项内容阻断；
- Codex 继续不修改项目负责人 Figma 几何，不进入 M1W 网页接入；
- 等待项目负责人完成剩余三项修改；12 区产品裁决随后已由 `DOC-MAP-12` 完成。
...
- Godot M0G 回归测试在 `4.7.1.stable.steam.a13da4feb` 下通过，输出 `M0G_TESTS_PASS count=4`；
- Git 空白检查通过，项目负责人新 ZIP、`.fig` 和美术素材保持未提交。
...
- Figma 已有 12 个地理分区，但项目文档仍有旧六分区模型，必须由项目负责人明确裁决后才能贯通修改。

### 阶段决定

- 插件交付结构门通过，但分区几何和 Brand 门未通过；
- Codex 不修正项目负责人几何，不进入 M1W 网页接入；
- 等待项目负责人修正 Figma、保存同版本 `.fig`、重新导出 ZIP，并确认 12 分区是否正式替代旧六区。
...
- 插件必须按项目负责人实际画布直接导出，不得因尺寸或缺少地图内 `00_BRAND` 阻止交付；
- `00_BRAND` 继续允许作为地图文件之外的独立项目素材。

### 本阶段实现

- 删除固定宽高的识别、验收和警告逻辑，保留旧主画框名称只用于向后兼容；
- 并列图层模式统一以 `06_FRAME` 为首选实际画布边界、`01_WATER` 为后备，不缩放或改写项目负责人矢量原稿；
...
- SVG 虽可无损缩放，但极复杂矢量仍可能使浏览器解析和绘制变慢；M1W 接入时必须实测加载、缩放和拖拽性能，必要时只优化网页运行副本，不修改项目负责人权威源文件；
- 不限制尺寸不等于允许各层坐标不一致：完整地图、底图、分区和对齐 SVG 的 `viewBox`、原点及结构 JSON 画布宽高仍必须完全一致；
- 项目负责人重新运行插件时应保持 PNG 选项关闭，先完成一次真实矢量 ZIP 导出验收。
...
- 原因是插件主线程不是普通浏览器页面，不能假定存在浏览器全局 `TextEncoder`；这与项目负责人的地图图层或素材内容无关；
...
- M0G 回归测试通过，Godot `4.7.1.stable.steam.a13da4feb` 输出 `M0G_TESTS_PASS count=4`；测试环境仍有既有 macOS 系统 CA 读取警告，但不影响项目测试；
- 本阶段仍需项目负责人重新运行插件完成一次真实 ZIP 导出，才能结束现场验收。

## TOOL-FIGMA-02 — 并列 Group 自动识别修复

### 问题与决定

- 项目负责人的七个地图顶层图层是同级 Group，而非被一个外层 Frame 包裹；旧版插件因此无法把已选中的 `06_FRAME` 识别为地图入口；
- 保留项目负责人现有 Figma 图层结构，不要求重新分组或改稿；
...
- 插件界面和项目交付规范同步更新，不再要求项目负责人为了导出而重构 Figma 原稿。
...
- 所有生产导出使用临时副本并在导出后删除，不改写项目负责人原图层；
...
- 本阶段只能完成本地静态和 ZIP 测试，必须由项目负责人在 Figma 桌面版导入插件并完成一次真实文件导出验收；
...
- Figma 地图顶层遮挡顺序以项目负责人批准的 `06_FRAME`、`05_NON_BUILDING_ORNAMENT`、`04_ROADS`、`03_DISTRICT_GEOMETRY`、`02_COASTLINE`、`01_WATER`、`00_BRAND` 为准。
...
- 当前导出标题缺少冒号，底部约有 `88 px` 透明余量，因此仍等待项目负责人从 Figma 重新导出修正版；
- 本次文档决策阶段不修改、移动或提交项目负责人的 `.fig` 和 SVG 源素材。

### 验证结果

- Git 空白检查与 Markdown 相对链接检查通过；
- 标题、署名、独立 Brand 和项目负责人批准的七层遮挡顺序已同步到总纲、产品、美术、验收、计划与素材流程；
- 未修改代码的 M0G 基线在 Godot `4.7.1.stable.steam.a13da4feb` 下通过，输出 `M0G_TESTS_PASS count=4`；
- 受限测试环境仍产生已知的 macOS 系统 CA 读取警告，但没有项目测试因此失败。

## DOC-L0 — 项目负责人审查文档语言纠偏

### 问题

- 项目把“面向英语玩家”错误扩大成了“开发文档使用英文”；
- 项目负责人当前需要审查的地图交付规范使用英文，导致审查成本和误解风险上升；
...
- 明确 `OWNER_PROJECT_REVIEW.md` 是中文项目负责人审查入口，并清理非必要英文管理用语；
- 在总纲和 `AGENTS.md` 中冻结语言边界：玩家内容英文，项目负责人审查与团队实施文档中文，稳定技术标识保留英文；
- 新增 `DOCUMENT_LANGUAGE_MIGRATION.md`，把其余英文规格分成四个受控翻译批次；
- 本阶段不改变玩法、数值、代码和 M1A 地图门禁。

### 当前门禁

完成静态检查、Godot 基线测试和独立 Git 提交后停止。项目负责人决定后续优先完成全部文档中文化，还是将翻译批次与后续里程碑交替安排；未获批准前不开始 M1A。
