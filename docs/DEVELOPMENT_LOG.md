...
- 按老板本轮授权直接优化现有网页。地图初始、最低和复位比例按实际视窗适宽计算；保留最高 1500% 与 500% 细节交互阈值。
...
- 新任务接手后重新读取项目规则、总纲、开发日志、老板审查计划、Git 状态和最近提交；
- 确认当前主线事实仍是 M1W 制作人网页原型，Godot 正式玩法和最终 UI 尚未启动；
- 记录工作区未跟踪素材分组和老板决策：旧 `m0_web/assets/cutouts/panel_center_large.png` 删除可保留，`references/` 未跟踪素材按长期参考处理，未归属 UI 素材暂不提交；
- 新增 M1W 网页视觉验收清单，下一步优先让老板审查当前网页地图、地块、地标、缩放、互斥选择和双语文案。
- 2026-09-13 老板确认：历史地标贴图已经接入，且 52 个历史地标摆在地图上的位置正确；后续不再把“历史地标位置”列为未确认项。

## M1W-HOTFIX-14 — 隐藏历史地标地图横幅

- 老板决定不再尝试修历史地标地图横幅可读性，要求直接去掉地图上的历史地标标签名；
...
- 自动测试和浏览器复查通过；其中横幅放大方案已在 `M1W-HOTFIX-14` 中被老板新决定取代，当前运行时不再显示历史地标地图横幅。

### 风险与后续门槛

- 未跟踪 UI 素材中可能包含 U1 或后续正式 UI 资产，但老板尚未决定哪些进入项目；
...
- 清除玩家可见的“老板绘制”“等待批准”“制作人演示”“正式制作输入”等内部工程身份文案；内部项目管理和审批记录继续留在相应文档中；
