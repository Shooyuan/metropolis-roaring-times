...
1. 地图本体街区标签使用老板指定字体 `Kings-Regular.ttf`。
2. 街区英文名按一个单词一行显示。
...
- 项目文档
  - 同步项目总纲、地图美术规范、验收标准、素材清单、老板审查计划和开发日志。
...
注意：本地浏览器如果已经缓存旧 `index.html`，普通 `http://127.0.0.1:4173/` 可能仍短暂读取旧的 `styles.css?v=m1w-home10`。本阶段已经把 `styles.css` 和 `localization.css` 都推进到 `?v=m1w-label1`；若老板当前页面仍显示旧字体，需要执行一次硬刷新。
