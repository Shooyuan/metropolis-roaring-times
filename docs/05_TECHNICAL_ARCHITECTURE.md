...
- LOD 服务根据固定缩放级别决定可命中类型：`100—477` 只开放街区识别/选择，历史地标插画仅用于识别；`500—1500` 开放地块与地标点击、开发建筑。历史地标地图横幅按项目负责人决定不渲染，名称与正文由右侧 Historical File 承载。
- 地标 `hover/focus/selected` 视觉状态必须作用于透明插画资产本体，例如网页原型中的 `.landmark-artwork`，不得使用父容器 `::after` 或普通边框绘制整张图片矩形框。悬停辉光随鼠标移出消失，选中辉光由统一选择状态保持。
- M1W 网页地块边框使用减半后的固定描边值：地块填充描边 `1.5px`、外轮廓 `3.5px`、内轮廓 `1px`。后续 Godot 或 U3 UI 可以重设样式，但必须保留“不得遮蔽地图细节”的验收意图。
...
项目负责人只执行一次插件导出；拆分生产文件由插件自动完成。插件 ZIP 作为不可替代的源交付归档，不由运行时直接读取。
