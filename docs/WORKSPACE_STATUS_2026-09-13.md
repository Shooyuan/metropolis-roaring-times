...
旧 Codex 任务因为上下文窗口耗尽无法继续压缩。本文件记录新任务接手时的工作区状态和项目负责人已确认的处理原则，避免后续窗口把未跟踪素材误当成已提交事实。
...
这些文件看起来属于最终游戏 UI、标题栏、状态栏或金融按钮方向，但项目负责人尚未决定哪些进入正式项目资产。
...
项目负责人确认这些按长期参考素材保留。未来若要提交，应先单独确认仓库体积和版权/来源说明。
