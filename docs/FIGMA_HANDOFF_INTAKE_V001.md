...
| PNG 缺席 | 通过 | 老板按要求保持 PNG 预览关闭；不影响 SVG 主交付 |
...
老板在 Figma 中需要做：展开 `03_DISTRICT_GEOMETRY`，打开 `district_soho` 的眼睛图标并保持可见，然后重新导出。
...
老板在 Figma 中需要做：只修改图层名称，不移动或改变形状。
...
老板在 Figma 中需要做：进入矢量编辑，连接 Chelsea 的首尾节点，使轮廓真正闭合；不要仅依赖填充视觉上自动封口。
...
老板在 Figma 中需要做：在 `Metropolis` 后加入冒号，重新导出独立透明 Brand SVG。
...
但现有项目交付规范仍保留早期六分区模型。Codex 不应擅自把 12 个地理分区合并回六区，也不应在没有老板确认时将 12 区直接写成最终游戏数据。

老板已于 2026-08-13 正式确认：以上 12 个分区替代旧六分区，作为 M1W 和后续游戏的权威分区集合。本报告保留原始冲突记录用于追溯。
...
- 当前 `.fig` 是 2026-08-12 的本地快照。老板完成上述修正后，必须再次执行 `File → Save local copy…`，使新的 `.fig` 与新 ZIP 成为同一修订版。
...
- `DOC-007A` 权威分区几何：**未通过**；
- Brand 标题验收：**未通过**；
- M1W 网页地图接入授权：**暂不进入**，等待老板修正并确认 12 分区裁决。
