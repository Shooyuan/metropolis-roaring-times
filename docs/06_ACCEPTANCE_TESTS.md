...
- 自动校验无错误后仍需项目负责人查看接触表与对齐页。项目负责人已于 2026-09-11 单独批准把待审运行素材接入 M1W 制作人网页，因此允许在网页中验证；这不等于批准进入 Godot，也不替代项目负责人对网页实际位置和可读性的最终验收。
...
At minimum, test an unowned, human, rival, government, selected, auction, construction, warning and violation plot. Pass when ownership and state are clear and click/hover/selection use the authoritative plot ID. In the M1W browser prototype, purchasable plot outlines must use the owner-approved thinner treatment: half of the previous visible stroke width, without hiding road or parcel detail.
...
- default zoom is `100%`;
- visible steps are exactly `100/125/156/195/244/305/381/477/500/625/781/977/1000/1250/1500%`;
- zoom-in and zoom-out move to adjacent approved steps;
- the target clamps exactly at `100%` and `1500%`;
...
4. 程序导入的地块数量恰好等于项目负责人交付的实际数量，代码和测试不存在固定数量常量。
...
12. 当前五张通用开发建筑 PNG 必须来自项目负责人交付素材，不得用 Codex 占位地理或随机图形冒充验收通过。
13. 玩家可见 HTML、语言包、历史正文和无障碍文案不得出现 `owner-authored`、`owner-approved`、`producer demo`、`项目负责人绘制`、`制作人地图演示` 等工程身份措辞；内部项目管理文档不受此限制。
