...
- 自动校验无错误后仍需老板查看接触表与对齐页。老板已于 2026-09-11 单独批准把待审运行素材接入 M1W 制作人网页，因此允许在网页中验证；这不等于批准进入 Godot，也不替代老板对网页实际位置和可读性的最终验收。
...
13. 玩家可见 HTML、语言包、历史正文和无障碍文案不得出现 `owner-authored`、`owner-approved`、`producer demo`、`老板绘制`、`制作人地图演示` 等工程身份措辞；内部项目管理文档不受此限制。
