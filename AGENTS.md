...
26. `08_PURCHASABLE_BLOCK_GEOMETRY` 中的 `cheap`、`medium`、`expensive` 是老板指定的地块初始价格层级。Codex 按地理顺序自动生成稳定地块 ID，并在批准区间内以固定种子生成可复现的精确基础地价；不得把 Figma 子层数字当运行时 ID，也不得在每局开始时重新随机价格。
